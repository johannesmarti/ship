...
farm = UncompetitiveFactory("farm", np.array([5,1,0,0]), 10)
mine = UncompetitiveFactory("mine", np.array([0,-0.5,1.5,0]), 10)
smith = UncompetitiveFactory("smith", np.array([0,-1,-2,2]), 20)
woodCutter = UncompetitiveFactory("wood cutter", np.array([0,5,0,-0.5]), 10)
...
def run_once(t : float):
    p = line_search_market(village, p0, epsilon, LineSearchConfiguration(t=t))
